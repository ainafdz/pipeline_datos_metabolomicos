...
ui <- fluidPage(
  ...
  tabPanel("Analisis multivariante", fluid=TRUE,
           tabsetPanel(
             tabPanel("PCA",
                      sidebarPanel(
                        helpText("NOTA: Se considera la imputación de missings KNN aunque no esté seleccionada, ya que el análisis PCA no se puede realizar si hay valores faltantes.")
                      ),
                      mainPanel(
             ...
             tabPanel("Heatmap", 
                      sidebarPanel(
                        helpText("NOTA: Si se selecciona el escalado de los datos se debe seleccionar también la imputación de valores faltantes.")
                      ),
                      plotOutput("heatmap")
                      ),
             tabPanel("PLS-DA",
  ...
  tabPanel("Integración con las características del terreno", fluid=TRUE,
           tabsetPanel(
             tabPanel("Características de las localizaciones",
                    mainPanel(
                      leafletOutput("mymap"))),
             tabPanel("Análisis de factores múltiples",
                                 sidebarLayout(
                                   sidebarPanel(
...
server <- function(input, output) {
  ...
  output$missingsText <- renderText({
    ...
    m <- round(apply(datos, 2, buscarNA),2)
    if(sum(m)<=0){
      print("No hay valores missing para esta selección.")
    }else{
      paste("En esta selección hay un total de", total_missings[1], "missings, que representan el", 
            total_missings[2], "% del total de datos de la selección.")
    }
  ...
  output$zeroText <- renderText({
    ...
    if(sum(n)<=0){
      print("No hay valores iguales a cero para esta selección.")
    }else{
      paste("En esta selección hay un total de", total_ceros[1], "valores cero, que representan el", 
            total_ceros[2], "% del total de datos de la selección.")
    }
  ...
  output$box_mult_desp <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu <- paretoscale(datos_impu)
    }
    
    boxplot(datos_impu,horizontal=F,names=FALSE )
  ...
  output$box_fin <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu4.1 <- paretoscale(datos_impu4.1)
    }
  ...
  output$box_var_fin <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu4 <- paretoscale(datos_impu4)
    }
  ...
  if (input$normscale == "Escala de Pareto") {
    ...
    datos_impu3 <- paretoscale(datos_impu3)
  }
  sumstats <- function(z) {
  ...
  output$descriptive3.1 <- DT::renderDataTable({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu3.1 <- paretoscale(datos_impu3.1)
    }
    mitj <- data.frame()
    med <- data.frame()
    des.st <- data.frame()
    ...
    for(i in 2:ncol(datos)){
      mitj <- rbind(mitj, aggregate(datos[,i], by=list(datos$Variedad),mean, na.rm=TRUE))
      med <- rbind(med, aggregate(datos[,i], by=list(datos$Variedad),median, na.rm=TRUE))
      des.st <- rbind(des.st, aggregate(datos[,i], by=list(datos$Variedad),sd, na.rm=TRUE))
    }
  ...
  output$anova <- DT::renderDataTable({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu5 <- paretoscale(datos_impu5)
    }
  ...
  output$anovatot <- DT::renderDataTable({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu6 <- paretoscale(datos_impu6)
    }
  ...
  output$scores_PCA <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu7 <- paretoscale(datos_impu7)
    }
  ...
  output$load_PCA <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu8 <- paretoscale(datos_impu8)
    }
  ...
  output$heatmap <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu9 <- paretoscale(datos_impu9)
    }
  ...
  output$plsda <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu10 <- paretoscale(datos_impu10)
    }
  ...
  output$corr_metab <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu11 <- paretoscale(datos_impu11)
    }
  ...
  output$corr_vinos <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu12 <- paretoscale(datos_impu12)
    }
  ...
  output$partialaxes12 <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu13 <- paretoscale(datos_impu13)
    }
  ...
  output$variables <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu14 <- paretoscale(datos_impu14)
    }
  ...
  output$individuals <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu15 <- paretoscale(datos_impu15)
    }
  ...
  output$puntos <- renderPlot({
    ...
    if (input$normscale == "Escala de Pareto") {
      ...
      datos_impu16 <- paretoscale(datos_impu16)
    }
